...
#define KEXPANED(km, a, m) do {\
  (m) = (m) >= 4? (m) + ((m) >> 1) : 16;\
  KREALLOC((km), (a), (m));\
} while (0)

#ifndef klib_unused
#if (defined __clang__ && __clang_major__ >= 3) || (defined __GNUC__ && __GNU__ >= 3)
...
#define KALLOC_POLL_INIT2(SCOPE, name, kmptype_t) typedef struct {\
  size_t cnt, n, max;\
  kmptype_t **buf;\
  void *km;\
} kmp_##name##_t;\
...
SCOPE void kmptype_t *kmp_alloc_##name(kmp_##name##_t *mp) {\
  ++mp->cnt;\
...
#define KALLOC_POOL_INIT(name, kmptype_t) KALLOC_POOL_INIT2(static inline klib_unused, name, kmptype_t)
